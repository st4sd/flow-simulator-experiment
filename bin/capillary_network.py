...
parser.add_argument('--replica_number',
                    type=int,
                    action='store',
                    metavar='REPLICA_NUMBER',
                    help='Replica number')
...
parser.add_argument('--plot_stats',
                    ...
                    metavar='PLOT_STATS',
                    default=False,
                    help='Plot network calculation statistics.')
...
with open(arg.filename, mode='r', encoding='utf-8') as file1:
...
while (abs(porosity_diff) > arg.epsilon):
    output_file_capillary_data = open(arg.out_folder + '/capillary_data.txt', mode='w')
    ...
    if arg.plot_stats:
        ...
        fig = op.topotools.plot_coordinates(network=pn, c='r', s=10, ax=fig)
    ...
    with open(arg.out_folder + '/centerlines.json', mode='w') as file2:
    ...
    # with open(arg.out_folder + '/new-' + arg.filename, mode='r', encoding='utf-8') as file3:
    with open(arg.out_folder + '/centerlines.json', mode='r', encoding='utf-8') as file3:
        data_pn = json.load(file3)
    ...
    if arg.plot_stats:
        ...
        hist_connectivity_pn, edgeshist_connectivity_pn = np.histogram(
                                        connectivity_pn, bins=bins_connectivity_pn, density=True)
        plt.hist(connectivity, bins=bins_connectivity, density=True, alpha=0.5, label='rock sample')
        plt.hist(connectivity_pn, bins=bins_connectivity_pn, density=True, alpha=0.5,
                 label='generated')
        plt.legend(loc='upper right')
        plt.xlabel(r'Capillary connectivity ')
        plt.ylabel(r'Probability Density')
    ...
    output_file_capillary_data.write(f'{arg.replica_number},{porosity_pn},'
                                     f'{graph_metadata_obj["number_of_nodes"]},'
                                     f'{graph_metadata_obj["number_of_links"]},'
                                     f'{int(xmax_pn)+1},{int(ymax_pn)+1},{int(zmax_pn)+1}')
    output_file_capillary_data.close()

    porosity_diff = porosity_rock - porosity_pn

    if abs(porosity_diff) > 5*arg.epsilon:
        if porosity_diff < 0:
            ds_ad = ds_ad + delta_ad*abs(10*porosity_diff)
        if porosity_diff > 0:
            ds_ad = ds_ad - delta_ad*(10*porosity_diff)
    else:
        if porosity_diff < 0:
            ds_ad = ds_ad + delta_ad*abs(3*porosity_diff)
        if porosity_diff > 0:
            ds_ad = ds_ad - delta_ad*(3*porosity_diff)
